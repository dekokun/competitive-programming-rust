...
fn main() {
    ...
    for _ in 0..h {
        let row: String = read();
        map.push(row.chars().map(|c| c == '.').collect());
    }
    let mut dp: Vec<Vec<ModInt1000000007>> = vec![vec![0.into(); w]; h];
    let mut x: Vec<Vec<ModInt1000000007>> = vec![vec![0.into(); w]; h];
    let mut y: Vec<Vec<ModInt1000000007>> = vec![vec![0.into(); w]; h];
    let mut z: Vec<Vec<ModInt1000000007>> = vec![vec![0.into(); w]; h];
