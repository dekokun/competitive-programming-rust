...
fn solve(mut a: Vec<usize>) -> usize {
    // aをソートしても回答は変わらないのでソート
    // ある数が何回最大になるか、何回最小になるかはソートされてると容易に計算可能(多分)
    // aに、何回最小になるかをかけたvec bを作る
    // 前計算として、 b の総和から、bの一つ一つを引いたらどうなるかの値を作っておく
    // aを順番に、それが最大のときの個数かける値を見ていき、aに、何回最小になるかをかけたvecをかけて足していく
    a.sort();
