...
fn main() {
    let n: usize = read();
    println!("{}", solve(read(), (0..n).map(|_| read()).collect()));
}

fn solve(k: u32, a: Vec<usize>) -> usize {
    let mut one = ModInt1000000007::new(0_usize);
    ...
    let mut two = ModInt1000000007::new(0_usize);
    for &v in a.iter() {
        for &v2 in a.iter() {
            if v2 > v {
                two += 1;
    ...
    (one * k + two * k * (k - 1) / 2).val() as usize
}
