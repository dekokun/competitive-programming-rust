...
fn main() {
    ...
    let k_minus_one_perm = perm_count(k - 1, k - 1);
    let mut tmp = ModInt1000000007::new(   1);
    for (i, &v) in vec.iter().enumerate().skip(1) {
        tmp *= i;
        if i < (k - 1) as usize {
            continue;
        }
...
fn perm_count(n: i64, k: i64) -> StaticModInt<Mod1000000007> {
    if n < k {
        return 0.into();
    }
