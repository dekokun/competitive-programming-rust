...
fn main() {
    ...
    for i in 0..h {
        for j in 0..w {
            if map[i][j] == '.' && counts[i][j].0 == 0 {
                let mut new_j = j;
                let mut count = 0;
                while new_j < w && map[i][new_j] == '.' {
                    count += 1;
                    new_j += 1;
                }
                for new_j in j..new_j {
                    counts[i][new_j].0 += count;
                }
            }
            if map[i][j] == '.' && counts[i][j].1 == 0 {
                let mut new_i = i + 1;
                let mut count = 0;
                while new_i < h && map[new_i][j] == '.' {
                    count += 1;
                    new_i += 1;
                }
                for new_i in i..new_i {
                    counts[new_i][j].1 += count;
                }
            }
        }
    }
    ...
    for line in &counts {
        for &count in line {
            ans += ModInt1000000007::new(2).pow(k);
            ans -= ModInt1000000007::new(2).pow(k - (count.0 + count.1) as u64);
        }
    }
    println!("{}", ans)
}
